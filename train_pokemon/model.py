...
if os.path.isfile(energy_model_filename):
	...
	feature_extractor.add(Convolution2D(3, 16, ksize=4, stride=2, pad=1, use_weightnorm=config.use_weightnorm))
	feature_extractor.add(Activation(config.nonlinearity))
	# feature_extractor.add(BatchNormalization(16))
	feature_extractor.add(dropout())
	feature_extractor.add(Convolution2D(16, 48, ksize=4, stride=2, pad=1, use_weightnorm=config.use_weightnorm))
	feature_extractor.add(Activation(config.nonlinearity))
	# feature_extractor.add(BatchNormalization(48))
	feature_extractor.add(dropout())
	feature_extractor.add(Convolution2D(48, 128, ksize=4, stride=2, pad=0, use_weightnorm=config.use_weightnorm))
	feature_extractor.add(Activation(config.nonlinearity))
	# feature_extractor.add(BatchNormalization(128))
	feature_extractor.add(dropout())
	feature_extractor.add(Convolution2D(128, 256, ksize=4, stride=2, pad=0, use_weightnorm=config.use_weightnorm))
...
if os.path.isfile(generative_model_filename):
	...
	model.add(Linear(config.ndim_input, 256 * input_size ** 2, use_weightnorm=config.use_weightnorm))
	model.add(Activation(config.nonlinearity))
	model.add(BatchNormalization(256 * input_size ** 2))
	model.add(reshape((-1, 256, input_size, input_size)))
	model.add(Deconvolution2D(256, 128, ksize=4, stride=2, pad=paddings.pop(0), use_weightnorm=config.use_weightnorm))
	model.add(Activation(config.nonlinearity))
	model.add(BatchNormalization(128))
	model.add(Deconvolution2D(128, 64, ksize=4, stride=2, pad=paddings.pop(0), use_weightnorm=config.use_weightnorm))
	model.add(Activation(config.nonlinearity))
	model.add(BatchNormalization(64))
	model.add(Deconvolution2D(64, 3, ksize=4, stride=2, pad=paddings.pop(0), use_weightnorm=config.use_weightnorm))
